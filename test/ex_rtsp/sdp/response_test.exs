defmodule ExRtsp.SDP.ResponseTest do
  ...
  test "decode/1 handle invalid responses" do
    assert {:error, _reason} = Response.read("header\r\n")
    assert {:error, _reason} = Response.read("header\r\n\r\nbody")
    assert {:error, _reason} = Response.read("header\r\nbody")

    assert {:error, _reason} =
             Response.read("some response\r\nsome invalid header\r\n\r\ninvalid body")
  end
  ...
  test "decode/1 decodes header and body" do
    ...
    assert %Response{} = resp = Response.read(resp)
  ...
  test "decode/1 decodes media" do
    ...
    assert %Response{media: media} = Response.read(resp)
